...
import SELECT_CARET from "./arrowDownLightGrey.svg";
...
function InteractiveDropdown({
    ...
    function renderSelectMenu() {
        ...
        function renderOption(value) {
            if (selection?.value === value?.value) {
                return (
                    <>
                        {isComponentOptions ? <div className="value-label" >
                            {value.html}
                        </div> : <div className="value-label" >
        ...
        return (
            <>
                {open && <div                               // TODO : Menu ...
                    ...
                    keepMounted={true}
                    ref={wrapperRef}
                    style={{ width: menuWidth ? menuWidth : "100%", border: "1px solid gray", padding: "5px", borderRadius: "5px", marginTop: "5px" }}
                ...
                >
                    ...
                    <div className="menu-list-div" style={{ maxHeight: "200px" }}>
                        ...
                        >
                            {({ index, style }) => {
                                return (
                                    <div className="menu-div" style={style} ref={
                                        isPagination ?
    ...
    function renderMultiSelectMenu() {
        ...
        function renderOption(value) {
            if (selection?.value === value?.value) {
                return <div className="value-label">{isComponentOptions ? <>{value.component}</> : value.label}</div>;
            }
            return <div className="value-label">{isComponentOptions ? <>{value.component}</> : value.label}</div>;
        }
                ...
                >
                    {isSearchable && (
                        <div style={{ width: "100%" }} className="menu-search-div">
                            <input
                                onChange={isServerSideSearch ? onSearchQueryChange : (e) => {
                                    setSearchText(e.target.value);
                                    const DisplayOptions = selectOptions
                                        .map((item) => {
                                            if (
                    ...
                    <div className="menu-container" style={{ maxHeight: "250px", overflowY: "auto", margin: "10px 0px 10px 0px" }}>
                        {displayOptions.length > 0 ? (
                                    ...
                                    <div ref={
                                        isPagination ?
                                            (displayOptions?.length === index + 1
                                                ? lastElementRef
                                                : null) : null
                                    } className="menu-div" key={index}>
                                        {(
                                            <div className={multiSelection.indexOf(item) > -1 ? "selected-styled-menu-div" : "styled-menu-div"} onClick={() => handleMultiSelect(item)}>
                                                {withCheckBox && (
                                                    <input
                                                        required={true}
                                                        type="checkbox"
                            ...
                            <div className="no-option-div" style={{ display: "flex", justifyContent: "center" }}>
